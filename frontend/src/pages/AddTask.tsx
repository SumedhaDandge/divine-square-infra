...
import { ArrowLeft, Phone, MessageCircle, MapPin, Bell, Mail, Users, Calendar as CalendarIcon, Clock, ChevronDown } from "lucide-react";
import { AppShell } from "@/components/layout/AppShell";
import { useCreateTask, TaskType } from "@/hooks/useTasks";
...
const taskSchema = z.object({
  ...
  type: z.enum(["call", "whatsapp", "site_visit", "reminder", "email", "meeting"]),
...
const taskTypes: { value: TaskType; label: string; icon: React.ElementType; color: string }[] = [
  { value: "call", label: "Call", icon: Phone, color: "bg-primary text-primary-foreground" },
  { value: "whatsapp", label: "WhatsApp", icon: MessageCircle, color: "bg-status-hot text-white" },
  { value: "site_visit", label: "Site Visit", icon: MapPin, color: "bg-accent text-accent-foreground" },
  { value: "reminder", label: "Reminder", icon: Bell, color: "bg-status-warm text-white" },
  { value: "email", label: "Email", icon: Mail, color: "bg-status-cold text-white" },
  { value: "meeting", label: "Meeting", icon: Users, color: "bg-status-new text-white" },
];
    ...
    <AppShell showFab={false} showBottomNav={false}>
      ...
      <form onSubmit={handleSubmit} className="flex-1 px-4 py-6 space-y-4">
        ...
        <div className="crm-card">
          <label className="text-sm font-semibold text-foreground mb-3 block">Task Type</label>
        ...
        {/* Title */}
        <div className="crm-card">
          <label className="text-sm font-semibold text-foreground mb-3 block">Task Title *</label>
          <input
            type="text"
        ...
        {/* Date & Time */}
        <div className="crm-card">
          <label className="text-sm font-semibold text-foreground mb-3 block">Schedule *</label>
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {formData.due_time || <span className="text-muted-foreground">Time</span>}
                </button>
              </PopoverTrigger>
                ...
                {timeSlots.map((time) => (
                  <button
                    ...
                    className={cn(
                      ...
                      formData.due_time === time && "bg-primary/10 text-primary"
                    )}
                  >
                    {time}
                  </button>
          ...
          {(errors.due_date || errors.due_time) && (
            <p className="text-destructive text-xs mt-1">{errors.due_date || errors.due_time}</p>
          )}
        </div>
