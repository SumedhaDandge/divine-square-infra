...
import { 
  ...
  Clock, FileText, Plus, ChevronRight, Building2, Wallet, Loader2
} from "lucide-react";
...
const getDisplayStatus = (status: any): "hot" | "warm" | "cold" | "new" | "converted" => {
...
export default function LeadDetail() {
  ...
  console.log("Lead Detail Rendered for ID:", id, "Lead Data:", lead);
  ...
  //     time: format(new Date(t.due_date), "MMM d, h:mm a"),
  ...
  //     title: `Site Visit - ${sv.projects?.name || "Project"}`,
  //     description: sv.notes || sv.status,
  //     time: format(new Date(sv.scheduled_date), "MMM d") + `, ${sv.scheduled_time}`,
  //     completed: sv.status === "completed",
  //     sortDate: new Date(sv.scheduled_date),
  //   })) || []),
  // ].sort((a, b) => b.sortDate.getTime() - a.sortDate.getTime());

  const quickActions = [
    { icon: Phone, label: "Call", color: "bg-primary text-primary-foreground", action: () => lead && window.open(`tel:${lead.phone}`) },
    { icon: MessageCircle, label: "WhatsApp", color: "bg-status-hot text-white", action: () => lead && window.open(`https://wa.me/${lead.phone.replace(/\s/g, "")}`) },
    { icon: MapPin, label: "Site Visit", color: "bg-accent text-accent-foreground", action: () => navigate(`/site-visits/new?leadId=${id}`) },
    { icon: FileText, label: "Quote", color: "bg-status-cold text-white", action: () => navigate(`/quotations/new?leadId=${id}`) },
  ];
      ...
      <header className="bg-primary text-primary-foreground px-4 pt-12 pb-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            {lead.email && <p className="text-primary-foreground/60 text-sm mt-1">{lead.email}</p>}
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 px-4 space-y-4 pb-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-foreground">{lead.interestedProject?.projectName || "Not specified"}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-foreground">{formatBudget(lead.budget.min, lead.budget.max)}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-foreground capitalize">
                  {lead.leadSource.name} • {format(new Date(lead.createdAt), "MMM d, yyyy")}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="crm-card">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              onClick={() => navigate(`/tasks/new?leadId=${id}`)}
          ...
          {/* {timeline.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              No activities yet. Add a task or schedule a site visit.
            </p>
          ) : (
              ...
              {timeline.slice(0, 5).map((item, index) => (
                <div key={item.id} className="flex gap-3">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {index < Math.min(timeline.length, 5) - 1 && (
